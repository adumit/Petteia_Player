...
if __name__ == "__main__":
    ...
    with tf.Session() as sesh:
        sesh.run(NNetwork.init_op)
        NNetwork.saver.restore(sesh, nn_saver_dir + sys.argv[1])
        ...
        while True:
            ...
            if user_choice == 'pr':
                ...
                while len([x for x in gameboardRed.pos if x]) > 0 and len([x for x in gameboardRed.neg if x]) > 0:
                    ...
                    if len(possible_moves) == 0:
                        break
                    move_scores = sesh.run(NNetwork.y_conv_red, feed_dict={
                        NNetwork.x_red: [gameboardRed.to_matrix(m) for m in
                                possible_moves]})
            ...
            if user_choice == 'pa':
                ...
                while len([x for x in gameboardRed.pos if x]) > 0 and len([x for x in gameboardRed.neg if x]) > 0:
                    ...
                    if len(possible_moves) == 0:
                        break
                    move_scores = sesh.run(NNetwork.y_conv_black, feed_dict={
